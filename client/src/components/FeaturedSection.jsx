...
const FeaturedSection = () => {
  ...
  return (
    <div className='px-6 md:px-16 lg:px-24 xl:px-44 overflow-hidden'>
        ...
        <div className='flex flex-wrap max-sm:justify-center gap-6 justify-center mt-8'>
            {dummyShowsData.slice(0,4).map((show)=>(
                <MovieCard key={show._id} movie={show}/>
            ))}
        </div>
        <div className='flex justify-center mt-20'>
            <button onClick={()=> {navigate('/movies'); scrollTo(0,0)}} className='px-10 py-3 text-sm bg-blue-500 text-white hover:bg-primary-dull transition rounded-md font-medium cursor-pointer'>Show more</button>
        </div>
