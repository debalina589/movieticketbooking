          ...
          {nowPlayingMovies.map((movie)=>(
            <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`} onClick={()=> setSelectedMovie(movie.id)}>
              ...
              {selectedMovie === movie.id && (
                <div className='absolute top-2 right-2 flex items-center justify-center bg-blue-700 h-6 w-6 rounded'>
     ...
     <div className='mt-8'>
      ...
      <div className='mt-6'>
        ...
        <div className='inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg'>
          ...
          <button onClick={handleDateTimeAdd} className='bg-rose-500 text-white px-3 py-2 text-sm rounded-lg hover:bg-red-500/10 cursor-pointer'>Add Time</button>
        </div>
      </div>
                ...
                <div className='font-medium'>{date}</div>
                  ...
                  {times.map((time) => (
                    <div key={time} className='border border-blue-300 px-2 py-1 flex items-center rounded'>
                      <span>{time}</span>
                      <DeleteIcon onClick={()=> handleRemoveTime(date, time)} width={15} className='ml-2 text-red-500 hover:text-red-700 cursor-pointer'/></div>
                    ))}
                </div>
              </li>
            ))}
      ...
      <button className='bg-rose-500 text-white px-8 py-2 mt-6 rounded hover:bg-rose-500/90 transition-all cursor-pointer'>Add Show</button>   
    </>
  ) : <Loading/>
}
